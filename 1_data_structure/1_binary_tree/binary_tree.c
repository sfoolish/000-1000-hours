...
{
	int arr[20] = {29, 23, 45, 16, 18,//测试数组
			17, 34, 38, 24, 19,
			0};
	bitree root;

	root = creat_tree(arr);//构建顺序二叉数遇到 0 结束
	printf("/******************************************/\n");
	inorder_bitree(root, print_data);//中序遍历二叉数
	printf("/******************************************/\n");
...
//		printf("root");
		(*visit)(root);
//		printf("left");
		inorder_bitree(leftp, visit);
//		printf("right");
		inorder_bitree(rightp, visit);
	}
}
...
//		scanf("%d", &temp);
		...
		if (temp != 0)	
		{
			...
			if (insert_des->data != temp)
			{
				...
				{
					printf("left\n");
					insert_des->l_child = node;//r_child -> l_child
				}
				else
				{
					printf("right\n");
					insert_des->r_child = node;//l_child -> r_child	
				}
			}
		}
...
node_t *search(bitree root, int key)
{
		...
		&&(tem_node->data != key))
	{
		...
		{
			p_node = tem_node;
			tem_node = tem_node-> l_child;//r_child -> l_child
		}
		else
		{
			p_node = tem_node;
			tem_node = tem_node-> r_child;//l_child -> r_child
		}
	}
